...
const formSchema = z.object({
  repoName: z
    ...
    .regex(/^[a-zA-Z0-9._-]+$/, "Please enter in the format owner/repo"),
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        toast.success(
          "Export started successfully. It may take a few minutes for the repository to be created.",
        );
      } catch (error) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <div className="flex items-center gap-1.5 h-full px-2 cursor-pointer text-muted-foreground border-l hover:bg-accent/30">
          {getStatusIcon()}
